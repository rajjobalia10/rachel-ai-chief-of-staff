...
- The product-led closing card uses the Orchid reference scale: 1200 × 675 px at large desktop, fluid down to an 80 px total viewport gutter, with the established 350 × 500 px mobile treatment. Every primary conversion CTA uses the exact Orchid treatment: a 134.43 × 50 px white pill, 20 px green iMessage tile, system-ui 14/14 medium label, subtle ink ring/shadow, and the label “Get Started.” The hero contains only this one action; never add a secondary hero CTA.
...
- Every “Get Started” and “Text Rachel” conversion CTA opens a direct `sms:` deep link to Rachel's production number, `+16282646604`, with `Hi Rachel` prefilled. Use Apple's `&body=` form on iPhone, iPad, and macOS, and `?body=` elsewhere. Do not restore an intermediate capture modal or claim the site can auto-send; the user always taps Send in Messages.
