...
test("the main conversion surface texts Rachel, and only pricing CTAs stay invite-only", async () => {
  ...
  assert.match(source, /const RACHEL_INVITE_CTA_LABEL = "Request an invite";/);
  assert.match(source, /src="\/assets\/imessage-icon\.png"/);
  assert.match(source, /href=\{invite \? "\/invite" : getRachelSmsHref\(\)\}/);
  assert.equal(source.match(/<MessagesCta(?:\s|\/|>)/g)?.length, 6);
  assert.match(source, /<MessagesCta invite \/>/);
  assert.match(source, /invite=\{standalone\}/);
